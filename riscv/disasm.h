...
extern const char* scr_name(int which);
...
class disassembler_t
{
 public:
  ...
  void add_ext_insn(disasm_insn_t* insn);

 private:
  mutable bool dirty = false;
  static const int HASH_SIZE = 256;
  mutable std::vector<const disasm_insn_t*> ext_chain[HASH_SIZE+1];
  mutable std::vector<const disasm_insn_t*> chain[HASH_SIZE+1];
};
